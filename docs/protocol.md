...
| `debug.activate` | *(none)* | `{ ok: true }` | Brings the app to the foreground on demand: reverses background-driven mode (`.accessory` → `.regular`), activates the app, deminiaturizes and surfaces a window. The one deliberate exception to the no-focus-steal rule — used by `harness.sh up --open`. In-process self-activation, no TCC permission. |
